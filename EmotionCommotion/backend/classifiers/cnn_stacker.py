...
X_train = pd.read_csv('combined_classifier/IEMOCAP_cnn_agg.csv')
X_test = pd.read_csv('combined_classifier/wild_frame_agg.csv')
# Read in labels
...
print(sum(X_test['session'] != y_test['session']))
...
for i in range(0,len(clf_funcs)):
	...
	np.savetxt('probs/' + clf_name + 'cnn_train.csv', train_probs, delimiter=',')
	np.savetxt('probs/' + clf_name + 'cnn_test.csv', test_probs, delimiter=',')
	pickle.dump(model, open('saved_classifiers/cnn_' + clf_name + '.pkl', 'wb'))