...
#Remove headers
X_old = X
X_new = pd.read_csv('./IEMOCAP_frame_agg.csv')
X_new = np.array(X_new)
X = np.concatenate([X_old, X_new], axis=1)
...
svm = GridSearchCV(SVC(class_weight='balanced'), tuned_parameters, cv=10,verbose=10,scoring=meanAcc, n_jobs = -1)
...
def plot_confusion_matrix(cm, title='SVM Confusion matrix', cmap=plt.cm.Greens,fontsize=14):
    ...
    plt.savefig("./cm_old_and_new.png",transparent=True,figsize=(20,20),dpi=120)
